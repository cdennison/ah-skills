...
Source: `src/skillspector/nodes/analyzers/semantic_security_discovery.py:30` (`ANALYZER_PROMPT`),
wrapped by `src/skillspector/llm_analyzer_base.py:227` (`BASE_ANALYSIS_PROMPT`).

Detects intent- and phrasing-based attacks that evade static regex detection.
`{file_label}` and `{numbered_content}` are filled in per file/chunk being scanned.
