#!/usr/bin/env python3
"""Map each pip package to the set of skills that mention installing it,
then append that as a comma-delimited last column on pip_security_report.csv.

Usage:
  python3 map_pip_packages_to_skills.py
"""

import csv
from pathlib import Path

from extract_pip_packages import extract_packages_from_line
...
OUT_CSV = HERE / "pip_security_report_with_skills.csv"
...
def main():
    if not REPORT_CSV.exists():
        print(f"Missing {REPORT_CSV} — run audit_pip_packages.py first.")
        return
